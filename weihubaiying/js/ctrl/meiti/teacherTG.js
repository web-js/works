...
var mrListId = getExtraDataByKey('mrListId');
...
//			window.webkit.messageHandlers.fenxiang.postMessage({name:mainVM.resources.nickName,con:mainVM.resources.content,url:encodeURI(mainVM.resources.headUrl),href:window.location.href});
			window.webkit.messageHandlers.rwfx.postMessage({name:mainVM.resources.nickName,con:mainVM.resources.content,url:encodeURI(mainVM.resources.headUrl),href:window.location.href});
		}
		;
		console.log(mainVM.resources.nickName,mainVM.resources.content,mainVM.resources.big_url,window.location.href+"&userInfo="+encodeURIComponent(JSON.stringify(userInfo)));
	});
...
mui('body').on('tap','#dial',function(){
	if(userInfo != ''){
		...
		mui.confirm('', '是否拨打客服电话？', btnArray, function(e) {
	    	if (e.index == 1) {
	        	window.location.href = 'tel:'+ mineInfo.phone;
	       	}else {                        
	        }
		})
	}else{
